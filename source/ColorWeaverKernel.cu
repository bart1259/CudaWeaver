...
__device__ bool colorInsideLine(float px, float py, float ax, float ay, float bx, float by, float lineThickness) {
    float vAPx = px - ax;
    float vAPy = py - ay;
    float vABx = bx - ax;
    float vABy = by - ay;
        ...
        for (size_t xa = 0; xa < SAMPLE_GRID_SIZE; xa++)
        {
            for (size_t ya = 0; ya < SAMPLE_GRID_SIZE; ya++)
            {
                if(((ya * SAMPLE_GRID_SIZE) + xa) % 5 == 0) {
                    ...
                    if(!colorInsideLine(pxaa, pyaa, ax, ay, bx, by, lineThickness)) {
                        val += 1.0f;
                    }
                    max += 1.0f;
                }
